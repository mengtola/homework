...
public class GroupCountries {
	...
	public static void main(String[] args) {
		...
		for(String item : getAllCountries()){		
			set.add(Character.toString(item.charAt(0)));
		}
		
		for(String key : set){
			List<String> lst = new ArrayList<String>();
			for(String val : getAllCountries()){
		...
		System.out.println("Element By Key (A)");
		System.out.println(ht.get("A"));
		System.out.println("Element By Key (B)");
		System.out.println(ht.get("B"));
		*/
		
		Collection<String> countlist = getAllCountries();
		Map<String,Collection<String>> countrymap = new HashMap<>();
		for(String item : countlist){
			
			String firstletter = String.format("%s", item.charAt(0)).toUpperCase();
			Collection<String> list = countrymap.get(firstletter);
			if(list == null){
				
				list = new HashSet<String>();
				countrymap.put(firstletter,list);
				list.add(item);
			}
		}
